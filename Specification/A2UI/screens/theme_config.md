...
🛠️ How to use this in A2UI Components

When AntiGravity builds the screens, it should import this config to ensure visual consistency.
...
const StatusBadge = ({ status }) => {
  const getStatusColor = () => {
    switch(status) {
      case 'Approved': return AIGU_THEME.colors.success;
      case 'Blocked': return AIGU_THEME.colors.error;
      default: return AIGU_THEME.colors.warning;
    }
  };

  return (
    <View style={{ backgroundColor: getStatusColor(), borderRadius: AIGU_THEME.borderRadius.sm }}>
      <Text style={AIGU_THEME.typography.caption}>{status}</Text>
    </View>
  );
};